...
def defrag_but_retain_coherency(file_combinations: list, parsed_files: list[tuple[str, str]]):
    for file_id in range(len(parsed_files) - 1, 0, -1):
        ...
        #enumerate(file_combinations)
        iterator = iter(file_combinations)
        #current_value = next(iterator)

        # move through new_values until we find a .
        # count forward equal to file length
        # if all values are .'s, we can move the file
        # else continue the search for the next dot
        # only move if we find a match
        current_id = 0
        for index, new_value in enumerate(file_combinations):
            ...
            #
            #     for i in range(file_length):
            #         file_combinations[index - 2 + i] = file_id
            #     var = True
            #     break



            #open_file_segment.append(new_value)
...
# Create tuples by pairing each element with the next, filling the last with '0' if needed
result = [(data[i], data[i+1] if i+1 < len(data) else '0') for i in range(0, len(data), 2)]
result
